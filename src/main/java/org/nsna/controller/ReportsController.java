		...
		if (isAuthorizedForReports(principle, passcode, passwd )) {		
			downloadDataAsCSV(response, 
				"SELECT CASE WHEN ( EDUAPPLICATION.STUDENT_ID in " 
					...
					+ "		WHEN  (a.REVIEWED_ANNUAL_FAMILY_INCOME <= 360000) then a.REVIEWED_ANNUAL_FAMILY_INCOME - 25000 ELSE "
					+ "		360000   end) /360000.0))  as decimal(5,4)) *.3) + "
					+ " 	(cast((a.REVIEWED_MARK_PERCENT /100.0) as decimal(5,4)) * .475) + "
					+ "	    (case when a.RETURNING_STUDENT = 'Y' then 1 else 0 end * .025) + "
					+ "		 ((isNull(a.REVIEWER_PREF_PERCENT, 0) /7.0) * .2) TotalScore, "
					...
					+ "	a.REVIEWER_PREF_PERCENT, "
					+ " a.EXCEPTION, "
					+ " a.REVIEWED_ANNUAL_TUTION_FEE, "
					+ "	case when REVIEWER_PREF_COMMENT like '%BL%' then 'BL' else null end BL, "
					+ "	case when REVIEWER_PREF_COMMENT like '%SP%' then 'SP' else null end SP, "
					+ "	case when REVIEWER_PREF_COMMENT like '%SS%' then 'SS' else null end SS, "
					+ "	case when REVIEWER_PREF_COMMENT like '%SB%' then 'SB' else null end SB, "
					+ " a.REVIEWER, "
